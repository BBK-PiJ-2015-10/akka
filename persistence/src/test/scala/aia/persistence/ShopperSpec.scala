...
import aia.persistence.Shopper.PayBasket
...
class ShopperSpec extends PersistenceSpec(ActorSystem("test"))
  ...
  "The shopper" should {
    ...
    "start with ane empty basket after payment, wallet intact" in {

      val shopperResurrected = system.actorOf(Shopper.props(shopperId),shopperName)
    ...
    "not be able spend more than the cash in the pocket " in {
