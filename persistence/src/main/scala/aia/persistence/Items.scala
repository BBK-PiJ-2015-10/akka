...
object Items {
  ...
  private def aggregateIndexed(indexed: List[(Item, Int)]) = {
    def grouped : Map[String,List[(Item,Int)]] = indexed.groupBy {
      case (item, _) => item.productId
    }


    def reduced : Iterable[(Item,Int)] = grouped.flatten { case (_, groupedIndexed) =>
      val init : (Option[Item],Int) = (Option.empty[Item],Int.MaxValue)
