...
class BasketEventSerializer extends Serializer {
  ...
  override def toBinary(obj: AnyRef): Array[Byte] = {
    obj match {
      case e: Basket.Event => BasketEventFormat.write(e).compactPrint.getBytes
      case msg => throw new Exception(s"Cannot serialize $msg with ${this.getClass} ")
    }
  }

  override def fromBinary(bytes: Array[Byte], manifest: Option[Class[_]]): AnyRef = {
    val jsonAst : JsValue = new String(bytes).parseJson
...
class BasketSnapshotSerializer extends Serializer {
  ...
  override def identifier: Int = 1242134234
...
object JsonFormats extends DefaultJsonProtocol {
  ...
  implicit val itemsFormat : RootJsonFormat[Items] =
    jsonFormat((list : List[Item]) => Items.aggregate(list), "items")
  ...
  implicit object BasketEventFormat extends RootJsonFormat[Basket.Event] {
    ...
    override def read(json: JsValue): Basket.Event = {
      json match {
        ...
        case j => deserializationError("Expected basket event, but got "+j)
      }
    }

    override def write(event: Basket.Event): JsValue = {
      event match {
