...
class Basket extends PersistentActor with ActorLogging {
  ...
  override def receiveCommand: Receive = {
    ...
    case RemoveItem(productId,_) =>
      if (items.containsProduct(productId)){
        persist(ItemRemoved(productId)){removed =>
          updateState(removed)
          sender() ! Some(removed)
        }
      } else {
    ...
    case UpdateItem(productId,number,_) =>
      if (items.containsProduct(productId)){
        persist(ItemUpdated(productId,number)){ updated =>
          updateState(updated)
          sender() ! Some(updated)
        }
      } else {
    ...
    case SaveSnapshotSuccess(metadata) =>
      log.info(s"Snapshot save with metadata $metadata")
