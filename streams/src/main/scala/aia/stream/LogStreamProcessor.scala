...
object LogStreamProcessor extends EventMarshalling {
  ...
  def parseLineEx(line: String): Option[Event] = {
    if(!line.isEmpty) {
      val input : Array[String] = line.split("\\|")
      input match {
        case Array(host :String, service: String, state : String, time : String, desc: String, tag: String, metric: String) =>
          val t = tag.trim
          val m = metric.trim
          Some(Event(
            host.trim,
        ...
        case Array(host: String, service: String, state: String, time :String, desc :String) =>
          Some(Event(
            host.trim,
            service.trim,
            state.trim match {
