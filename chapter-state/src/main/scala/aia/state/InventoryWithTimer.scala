...
class InventoryWithTimer(publisher: ActorRef) extends Actor
  ...
  when (WaitForPublisher, stateTimeout = FiniteDuration(5,TimeUnit.SECONDS)) {
  ...
  onTransition{
    ...
    case _ -> ProcessSoldOut => {
      nextStateData.pendingRequests.foreach(request => {
        request.target ! new BookReply(request.target,Left("SoldOut"))
      })
      self ! Done
    }
