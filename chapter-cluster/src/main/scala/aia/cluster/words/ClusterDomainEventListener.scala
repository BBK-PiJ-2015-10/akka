...
import akka.cluster.ClusterEvent.{MemberUp,MemberExited,MemberRemoved,UnreachableMember,ReachableMember,CurrentClusterState,ClusterDomainEvent}
  ...
  override def receive: Receive = {
    ...
    case MemberRemoved(member, previousStatus) =>
      if (previousStatus == MemberStatus.exiting){
        ...
        log.info(s"$member Previously down after unreachable, REMOVED")
      }
