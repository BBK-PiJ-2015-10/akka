...
class JobMaster extends Actor
  ...
  def idle: Receive = {
    case StartJob(jobName, text) =>
      ...
      val cancellable = context.system.scheduler.schedule(Duration(0,TimeUnit.MILLISECONDS),Duration(1000,TimeUnit.MILLISECONDS), router, Work(jobName, self))
      context.setReceiveTimeout(Duration(60,TimeUnit.SECONDS))
